        ...
        switch(side.index) {
          case 0:
            ...
            if (this.cubeLayers.indexOf(0) !== -1) {
              side.material.color.setHex(0xEEEEEE);
            }
            break;
          case 1:
            side.rotation.y = THREE.Math.degToRad(-90);
            if (this.index % 3 == 0) {
                side.material.color.setHex(0xEF6803);
            }
            break;
          case 2:
            if (this.index > 5) {
                side.material.color.setHex(0x2CB22C);
            }
            break;
          case 3:
            side.rotation.y = THREE.Math.degToRad(90);
            if (this.index % 3 == 2) {
                side.material.color.setHex(0xBA232E);
            }
            break;
          case 4:
            side.rotation.y = THREE.Math.degToRad(-180);
            if (this.index < 3) {
                side.material.color.setHex(0x3372CF);
            }
            break;
          case 5:
            side.rotation.x = THREE.Math.degToRad(90);
            if (this.cubeLayers.indexOf(5) !== -1) {
              side.material.color.setHex(0xEECE02);
            }
            break;
        }
        side.translateZ(5);
...
async function turnFace(face, direction, showanimation=true) {
  switch (direction) {
  ...
  switch(pivotSide) {
    case 0: {
      await animateFaceTurn(topLayer, -direction, yAxis, showanimation);
      await snap();
    ...
    case 5: {
      await animateFaceTurn(bottomLayer, -direction, yAxis, showanimation);
      await snap();
    ...
    case 1: {
      await animateFaceTurn(leftLayer, direction, xAxis, showanimation);
      await snap();
    ...
    case 3: {
      await animateFaceTurn(rightLayer, -direction, xAxis, showanimation);
      await snap();
    ...
    case 2: {
      await animateFaceTurn(frontLayer, -direction, zAxis, showanimation);
      await snap();
    ...
    case 4: {
      await animateFaceTurn(backLayer, direction, zAxis, showanimation);
      await snap();
...
async function animateFaceTurn(faceObject, direction, axis, showanimation) {
  snapping = true;
  let accel = 0;
  for (let i = 0; i < 25; i++) {
    if (i < 13) {
    ...
    if (showanimation) await timer(7);
  }
  snapping = false;
}
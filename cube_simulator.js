...
function cube_notation(move){
    ...
    }else if (['F','F\''].includes(notation)){
        let dynamic_cube = structuredClone([cube[0],cube[2],cube[5],cube[4]]);
        let dynamic_cube_copy = structuredClone(dynamic_cube);
        let direction = -1;
        if (notation == 'F\'') direction = 1;
        for (let j = 0; j < 3; j++){
            dynamic_cube[0][2][j] = dynamic_cube_copy.at(0+direction)[j][2];
            dynamic_cube[1][j][0] = dynamic_cube_copy[1+direction][2][j];
            dynamic_cube[2][0][j] = dynamic_cube_copy[2+direction][j][0];
            if (3 + direction == 4) dynamic_cube[3][j][2] = dynamic_cube_copy[0][0][j]
            else dynamic_cube[3][j][2] = dynamic_cube_copy[3+direction][0][j];
        }
